...
- نموذج لغوي محلي بلا مفتاح لتصنيف الطلب واختيار مرجعه مع قواعد سلامة ومراجعة بشرية / No-key browser LLM classification and reference selection with server safety rules
...
الافتراضي الآن **نموذج Qwen3‑1.7B محلي مجاني داخل المتصفح**، دون حساب مزوّد أو مفتاح API.
افتح `/local-model` لتجربته دون تسجيل دخول. في صفحة الصيانة يكون خيار النموذج مفعّلًا؛
يقترح فئة العطل ويختار مرجعًا مناسبًا للأعراض، أو يطلب توضيحًا/مراجعة نطاق الطلب.
تأتي نصوص الحلول من المراجع التي يقبلها الخادم، وتبقى الخطورة من قواعد السلامة.
التنزيل الأول يقارب 1.1 غيغابايت، بصيغة GGUF Q4_K_M عبر wllama 3.6.1. يتطلب متصفحًا حديثًا يدعم WebAssembly Memory64 وذاكرة متاحة. يستخدم WebGPU عند توفر محوّل رسومي مناسب ودعم JSPI، مع محاولة واحدة على المعالج عند فشل المسار الرسومي. عند تعذره أو إلغائه
...
يوفّر المحرك خانتين للسياق، ويختار بينهما تلقائيًا بحسب تشابه بداية التعليمات لتقليل إعادة معالجتها عند الانتقال بين التصنيف واختيار المرجع. هذا يختلف عن إعادة استخدام نتيجة بلاغ مطابق داخل الجلسة؛ لا تُحسب النتيجة المعاد استخدامها قياسًا لسرعة الاستدلال الجديد. قياس المقارنة القابل للإعادة: `cd frontend && npm ci && npm run build && npx playwright install chromium && npm run benchmark:llm -- --compare-runtimes --low-storage`.
